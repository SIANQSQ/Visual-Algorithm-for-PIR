...
class PIL_App:
    ...
    def parse_4_packclass(self, packet: bytes, byteorder: str = 'little') -> int:
        ...
        return int.from_bytes(packet[0], byteorder=byteorder, signed=True)
    ...
    def parse_4_shotnum(self, packet: bytes, byteorder: str = 'little') -> int:
        ...
        if len(packet) != 4:
            raise ValueError('解析距离出错:数据包长度必须为4字节')
        return int.from_bytes(packet[3], byteorder=byteorder, signed=True)
                        ...
                        if data:
                            if len(data) == 4:
                                try:
                                    ...
                                    PIL_Diatance = self.parse_4_diatance(data, byteorder='little')
